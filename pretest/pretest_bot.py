...
def analyze(pair, since):
    ...
    server_time_unix = requests.get('https://api.kraken.com/0/public/Time').json()['result']['unixtime']
    server_time_normal = requests.get('https://api.kraken.com/0/public/Time').json()['result']['rfc1123']
    #print(time_now)
    #print(server_time_unix)
    ...
    # print current balance, last_trade (= Buy) and last trades (= Buy) price
    ...
    try:
        ...
        selling_point_win = last_trade_price * 1.0005
        selling_point_loss =  last_trade_price * 0.9995
        ...
        if open_ >= selling_point_win or close_ >= selling_point_win:
            ...
            # print previous balance, last_trade (= Buy) and last trades (= Buy) price as well as the current candle and that it was sold as well as the new balance
    ...
    if not did_sell and float(balance['USD.HOLD']) > 0:
        ...
        if highest/lowest >= price_to_buy and low_ <= lowest:
            ...
            # print previous balance, last_trade (= Buy) and last trades (= Buy) price as well as the current candle and that it was bought as well as the new balance
            print('Balance: ' + json.dumps(balance, indent=4) + '\n\nLast Trade: ' + json.dumps(last_trade, indent = 4) + '\n\nLast Trade Price: ' + json.dumps(last_trade_price, indent=4))
            print('Current candle:\nO: ' + str(open_) + '\nH: ' + str(high_) + '\nL: ' + str(low_) + '\nC: ' + str(close_))
            print('\n')
    ...
    print('last iteration at: ' + server_time_normal + '\nCurrent balance: ' + str(get_fake_balance()) + '\n')
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        analyze(pair, since)
        print('Sleep for {} seconds...'.format(sleeping_time))
